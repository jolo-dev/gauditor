...
import * as s3 from "@npm-audit/core/s3";
// import * as createOutput from "@npm-audit/core/create-output";
...
vi.mock("@npm-audit/core/s3", () => {
  return {
    getReportByKey: mockedGetReportedByKey,
  };
});

vi.mock("@npm-audit/core/create-output", () => {
  return {
...
describe("parse-reports", () => {
  ...
  it("should call the handler", async () => {
    ...
    const s3Spy = vi
      .spyOn(s3, "getReportByKey")
      .mockResolvedValue(JSON.stringify({ vulnerabilities: { "repo1": {} }, metadata: {vulnerabilities: {high: 1, critical: 1}} } ));
